...
def is_valid_package(mod_package):
    return mod_package is not None and isinstance(mod_package, modbase.ModPackage)

def is_valid_mod(mod):
    return mod is not None and isinstance(mod, modbase.ModBase)

# TODO: Should these be moved to modebase?
def is_valid_patch(patch):
    return (patch is not None and isinstance(patch, modbase.Patch)
...
def is_valid_dependency(dependency):
    return (dependency is not None and isinstance(dependency, modbase.Dependency)
